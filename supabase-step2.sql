-- Step 2: Create all tables, functions, policies
-- Run AFTER the cleanup step succeeds
...
begin
  ...
  if not exists (select 1 from pg_publication_tables where pubname = 'supabase_realtime' and tablename = 'columns') then
  ...
  if not exists (select 1 from pg_publication_tables where pubname = 'supabase_realtime' and tablename = 'card_labels') then
    alter publication supabase_realtime add table card_labels;
  end if;
  if not exists (select 1 from pg_publication_tables where pubname = 'supabase_realtime' and tablename = 'card_members') then
